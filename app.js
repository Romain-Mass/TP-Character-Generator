...
const $btCreate = document.querySelector("#btnCreate");
...
function openModal() {
	...
	const checkedRadio = document.querySelector(".class-option:checked");
	if (checkedRadio && SVG_CHARS[checkedRadio.value]) {
		$previewArt.innerHTML = SVG_CHARS[checkedRadio.value];
		$previewClass.textContent = CLASS_LABELS[checkedRadio.value];
	}
}
...
$btCreate.addEventListener("click", openModal);
...
$classOptions.forEach((radio) => {
	radio.addEventListener("change", () => {
		if (radio.checked) {
			...
			if (SVG_CHARS[selectedClass]) {
				$previewArt.innerHTML = SVG_CHARS[selectedClass];
			}
			$previewClass.textContent = CLASS_LABELS[selectedClass];
		}
	});
});

$inputName.addEventListener("input", () => {
	$previewName.textContent = $inputName.value || "Hero";
});

$inputStr.addEventListener("input", () => {
	$strVal.textContent = $inputStr.value;
	const pct =
		(($inputStr.value - $inputStr.min) / ($inputStr.max - $inputStr.min)) * 100;
	$inputStr.style.setProperty("--pct", pct + "%");
...
$inputLife.addEventListener("input", () => {
	...
	const pct =
		(($inputLife.value - $inputLife.min) / ($inputLife.max - $inputLife.min)) *
		100;
	$inputLife.style.setProperty("--pct", pct + "%");
...
$charForm.addEventListener("submit", (e) => {
	...
	const selectedClass = document.querySelector(".class-option:checked").value;

	$cardArt.innerHTML = SVG_CHARS[selectedClass];
	$cardName.textContent = $inputName.value || "Hero";
	$cardClass.textContent = CLASS_LABELS[selectedClass];
	$cardStr.textContent = $inputStr.value;
	$cardLife.textContent = $inputLife.value;
